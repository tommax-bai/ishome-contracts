...
- **零生成**：`lockedTextsByDomain` 是**锁定清单**（图 v0.2 §2）——本产物必挂的锁定文案 ID 集，
  **只有 ID 没有正文**（规则 2.4 gen-locked：引用 ID 直接渲染），枚举与"产物→必挂文案"映射见
  `../registries/locked_texts.md`。要挂哪几条由调用方按它在生成哪个 `art-` 传入（同 `entitlement`
  机制，规则引擎不持有产物清单）；挂载在成文线装配层，齐不齐由册级校验判。

字段只增不删；锁定清单已落地（`lockedTextsByDomain`，非必填——生产方未升级的旧包缺省无必挂文案），
动作表字段随清单求值落地时增补。
