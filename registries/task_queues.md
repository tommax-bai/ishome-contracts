...
|---|---|---|---|
| `genpipe-activities` | genpipe-worker（ishome-aipipe） | `floorplan-parse`、`plan-layout-solve`、`plan-rule-check`、`consistency-check`、`compliance-check`、`floorplan-geometry-extract`、`plan-notes-write`、`plan-copy-write`、`task-result-deliver`（后四条 2026-09-04 入册：解析/事实/批注/文案属非绘图 activity，回流属编排收尾） | 对齐文档 §3.1（编排与非绘图 activity 留 genpipe） |
| `render2d-activities` | render2d-svc（ishome-render2d） | `plan-2d-render`、`style-caption-overlay`（2026-09-04 入册：确定性叠字与母版同伸缩轴） | V1.4 裁决：确定性 2D 绘图，CPU 伸缩轴 |
