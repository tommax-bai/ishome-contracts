...
> 机器可读投影：`locked_texts.json`（渲染层按 ID 逐字取正文的数据源；本表仍是叙事权威，
> 一致性由 `scripts/check_locked_texts_sync.py` 在 pre-push 强制；**不编进 `gen/` 生成代码**——
> 写作层依赖面只含生成代码，正文对写作层保持结构性不可见）。
...
| `DISCLAIM_APPENDIX` | **待补录：由人出具的一份法务说明（禁编造、禁 AI 补写）** | 报告册尾附录，`stage-project` 全册必含：**册内免责与安全话术的唯一载体**（规范规则 7.1 / v2.6 裁决 2026-08-30——原七条逐条嵌页改判为一份附录整体承接） | 仅 ID 与用途（待补录）|
...
**v2.6 改判说明（裁决 2026-08-30，规范规则 7.1）**：用户裁决——册内免责*"由人员出一个法务说明，放在附录里面就行了"*，
且该事项**排在全部代码事项之后**。三点落地：①新注册 `DISCLAIM_APPENDIX` 为**册内免责的唯一载体**，正文＝人一次性出稿的
一份法务说明，逐字置于册尾附录，gen-locked 三条纪律不变（变的是数量七条→一条、位置页内→册尾）；
②五条手册项（`SAFE_WALL_*`／`SAFE_LAYOUT_CHANGE`／`DISCLAIM_RENDER`／`DISCLAIM_PRICE`）**ID 与正文不动**（只增不改），
改的是**挂载条件**——报告册正文页不再逐条必挂，**图框侧（`prec-schematic`／`prec-concept`）等绘图线出第一张图纸时另议**；
③两条 `*_SITE_CHECK` 新 ID 经用户判定为**提醒不是免责**（*"保留吧 这个算提醒"*），**保留原挂载位置**、
**不进法务批次**，正文由产品侧定稿走常规改源路径（规则 4.19），随"定位数字归类重标"同批落表。
**交付前置**：报告册对外交付业主＝`DISCLAIM_APPENDIX` 有正文；落表前报告只作内部验证与产品迭代用。
...
| `DISCLAIM_APPENDIX` | 报告册尾附录（`stage-project` 全册必含） | 规范规则 7.1 + 裁决 2026-08-30 | 裁决明文，**正文待补录（人出稿）** |
| `DISCLAIM_PRICE` | ~~`art-budget-chapter`~~ **册内改由 `DISCLAIM_APPENDIX` 承接（v2.6）**；图纸／图框侧不适用本条 | 规则 5.15 原文"DISCLAIM_PRICE 必挂"（作对照留存）＋ 规范规则 7.1 改判 | **规范明文，挂载条件已改判** |
...
- ~~**七条待补录正文同批走起草 + 法务**（五条手册项 + 两条新 ID）：手册原文找不到（裁决②），法务过目后一次落表~~
  **已改判（2026-08-30，见上「v2.6 改判说明」）**：法务批次收敛为 `DISCLAIM_APPENDIX` 一条（人出稿，排在全部代码事项之后）；
  两条 `*_SITE_CHECK` 转由产品侧定稿、随定位数字归类重标落表；五条手册项正文仍待补录但**册内不再必挂**，
  其正文与产物集**等绘图线出第一张图纸时**连同图框侧挂载一并收口。落表前任何消费方仍只拿得到 ID。
